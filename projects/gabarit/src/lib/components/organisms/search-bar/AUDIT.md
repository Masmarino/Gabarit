...
| 10.7      | Visible focus indicator                        | `.gbt-sb-trigger__input` sets `outline: none` with no `:focus-visible` re-add — its own outline used to double up on a second, separate ring already drawn one level up. `[class.gbt-sb-trigger--open]="isFocused()"` puts the border-color change and `box-shadow` glow on `.gbt-sb-trigger` in lockstep with the input's actual focus state (bound to `(focus)`, not gated to keyboard-only like `:focus-visible`), so the container ring alone is always present exactly when the field is focused — by mouse or keyboard, unlike the outline it replaced.                          | Compliant                        |
