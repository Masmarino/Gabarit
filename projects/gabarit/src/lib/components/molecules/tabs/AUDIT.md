...
| 4.1        | Compatible with assistive technology (structural)       | `activeIndex` moving from a plain internal signal to an Angular `model()` changes no markup, ARIA attribute, or event handling — the ARIA tab pattern (`role`, `aria-selected`, `aria-controls`, roving `tabindex`) is entirely unaffected. Verified by the full pre-existing test suite passing unchanged (`tabs.spec.ts`).                                                                                                                                                                                                                                | Compliant — unaffected                     |
